-- 003 platform layer + ranchrodeo_runs for ranchrodeo-mobile-app
...
-- Discipline run log for ranch rodeo.
create table if not exists public.ranchrodeo_runs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  event_type text,
  member_1 text,
...
create index if not exists ranchrodeo_runs_user_idx on public.ranchrodeo_runs (user_id, created_at desc);

alter table public.ranchrodeo_runs enable row level security;

drop policy if exists "ranchrodeo_runs_owner_all" on public.ranchrodeo_runs;
create policy "ranchrodeo_runs_owner_all" on public.ranchrodeo_runs
