...
// analyze-video edge function for ranch rodeo.
//
...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    phases: {
      type: "object",
      additionalProperties: false,
      required: ["horsemanship", "cattle_handling", "teamwork", "efficiency"],
      properties: {
          horsemanship: { type: "string" },
          cattle_handling: { type: "string" },
...
const SYSTEM_PROMPT =
  "You are an expert ranch rodeo coach. Analyze the athlete's run from the supplied frames. " +
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM_PROMPT + " JSON schema: " + JSON.stringify(ANALYSIS_SCHEMA) },
          {
            role: "user",
            content: [
              { type: "text", text: "Analyze this ranch rodeo run." },
              ...images,
            ],
          },
        ],
    ...
    if (supabaseUrl && serviceKey && user_id) {
      ...
      await admin.from("video_analysis_results").insert({
        user_id,
        event_type: body.event_type ?? "ranchrodeo",
        video_url,
